...
public class AddTagsModule extends VBox {
    ...
    public AddTagsModule(TagGroupDto groupDto) {
        super();
        this.setMinWidth(150);
        this.categoryId = groupDto.id();
        this.name = new Label(groupDto.name());
        this.tags = groupDto.tags().stream().map(TagDto::name).map(CheckBox::new).collect(Collectors.toList());
        this.ordering = groupDto.ordering();
        tagList = new VBox();
